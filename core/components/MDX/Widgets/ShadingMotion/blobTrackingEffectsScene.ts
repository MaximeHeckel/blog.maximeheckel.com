...
import PostprocessingCode from './blobTrackingEffectsPostprocessing';
...
const MotionBoxes = () => {
  ...
  const { effectType } = useControls('Effects', {
    effectType: {
      options: {
        Dithering: 0,
        Thermal: 1,
  ...
  const { segmentType } = useControls('Connectors', {
    segmentType: {
      options: {
        Straight: 0,
        Curved: 1,
  ...
  const outputNode = useMemo(
    () =>
      createOutputNode({
        effectType: shaderUniforms.effectType,
        maxBlobs: MAX_BLOBS,
        resources,
        segmentType: shaderUniforms.segmentType,
        targetAspect,
  ...
  useFrame((state) => {
    ...
    shaderUniforms.effectType.value = effectType;
    shaderUniforms.hasPreviousFrame.value = hasPreviousFrameRef.current;
    shaderUniforms.segmentType.value = segmentType;
    shaderUniforms.time.value = state.clock.elapsedTime;
...
export default BlobTrackingEffects;
