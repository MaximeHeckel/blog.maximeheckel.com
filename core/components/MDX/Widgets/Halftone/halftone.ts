...
class HalftoneEffectImpl extends Effect {
  constructor({ pixelSize = 1.0, offset = true, useLuma = true }) {
...
const HalftoneEffect = () => {
  const { enabled, pixelSize, offset, useLuma } = useControls({
    enabled: {
      value: true,
      label: 'Halftone',
    },
  ...
  return (
    <EffectComposer enabled={enabled}>
      <CustomHalftoneEffect pixelSize={pixelSize} offset={offset} useLuma={useLuma} />
    </EffectComposer>
  );
};
