...
vec3 depthToViewPosition(vec2 uv, float depth) {
  vec3 ndc = vec3(uv * 2.0 - 1.0, depth * 2.0 - 1.0);
  vec4 view = projectionMatrixInverse * vec4(ndc, 1.0);
  return view.xyz / view.w;
}

float depthToRayDistance(vec2 uv, float depth) {
  return length(depthToViewPosition(uv, depth));
}

vec3 getWorldPosition(vec2 uv, float depth) {
  vec3 viewPos = depthToViewPosition(uv, depth);
