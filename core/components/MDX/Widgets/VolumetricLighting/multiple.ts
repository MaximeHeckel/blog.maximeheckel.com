...
void mainImage(const in vec4 inputColor, const in vec2 uv, out vec4 outputColor) {
  ...
  for (int i = 0; i < NUM_STEPS; i++) {
    ...
    float density = -sdfVal + fbm(samplePos);
    ...
    vec3 luminance = lightColor * LIGHT_INTENSITY * attenuation * scatterPhase * shadowFactor;
...
const AppCode = `import { OrbitControls, PerspectiveCamera, useFBO, useTexture } from '@react-three/drei';
...
export const VolumetricLightEffect = () => {
  ...
  const { lightPosition, coneAngle, shadowMapSize, enabled } = useControls({
    lightPosition: {
      value: {x: 0, y: 0, z: -7},
  ...
  const lightDirection = useRef(new THREE.Vector3().copy(lightPosition).negate().normalize());
  ...
  useFrame((state) => {
    ...
    const currentLightTargetPos = new THREE.Vector3().addVectors(lightPosition, lightDirection.current);
    ...
    if (effectRef.current) {
      ...
      effectRef.current.lightPosition = new THREE.Vector3(lightPosition.x, lightPosition.y, lightPosition.z);
  ...
  return (
    <>
      <spotLight 
        ref={lightRef} 
        castShadow
        position={lightPosition.current} 
        intensity={100.0} 
        angle={coneAngle * Math.PI / 180.0 } 
        penumbra={0.35}
      />
  ...
  return (
    <>
      <PerspectiveCamera
        ...
        far={200}
      />
      <mesh castShadow receiveShadow position={[0, -1, -1]} rotation={[-Math.PI / 2, 0, 0]}>
        <planeGeometry args={[50, 50]} />
        <meshStandardMaterial color='black' />
      </mesh>
      <mesh castShadow receiveShadow position={[0, 0, -1.5]} rotation={[0, -Math.PI / 2, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[-3, 0, -1]} rotation={[0, Math.PI / 4, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[3, 0, -1]} rotation={[0, -Math.PI / 5, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[2, 0, 1]} rotation={[0, Math.PI / 3, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[-2, 0, 0.75]} rotation={[0, 0, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[1.0, 0, 3.5]} rotation={[0, 0, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[4, 0, 4.25]} rotation={[0, Math.PI / 3, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
      <mesh castShadow receiveShadow position={[-1.75, 0, 4.25]} rotation={[0, -Math.PI / 4, 0]}>
        <boxGeometry args={[1.5, 2.5, 0.5]} />
        <meshStandardMaterial color='white' />
      </mesh>
