...
const SHAPES = ['Cylinder', 'Cone'];
const SHADOW_BIAS = 0.0001;
const SHAPE_TYPE = 1.0;

export const VolumetricLightEffect = () => {
  const effectRef = useRef();
  const lightRef = useRef();
  const { autoRotate, showLightCamera, shadowMapSize, coneAngle, lightDirection, enabled } = useControls({
    autoRotate: {
      value: true,
      label: 'Auto Rotate',
    },
  ...
  const animatedLightPosition = useRef(new THREE.Vector3(4.0, 4.0, -4.0)); 
  const animatedLightDirection = useRef(new THREE.Vector3());
  ...
  useFrame((state) => {
    ...
    if (autoRotate) {
      animatedLightPosition.current.x = Math.cos(angle) * 4.0;
      animatedLightPosition.current.z = Math.sin(angle) * 4.0;
      animatedLightDirection.current.copy(animatedLightPosition.current).negate().normalize();
    }

    if (lightRef.current) {
      lightRef.current.position.copy(animatedLightPosition.current);
    }

    const currentLightTargetPos = new THREE.Vector3().addVectors(animatedLightPosition.current, animatedLightDirection.current);

    lightCamera.position.copy(animatedLightPosition.current);
    ...
    if (effectRef.current) {
      ...
      effectRef.current.lightDirection = animatedLightDirection.current;
      effectRef.current.lightPosition = animatedLightPosition.current;
      effectRef.current.shapeType = SHAPE_TYPE;
  ...
  return (
    <>
      {showLightCamera && (
        <cameraHelper args={[lightCamera]} />
      )}
      <directionalLight ref={lightRef} position={animatedLightPosition.current} intensity={3.0} />
