...
const float NOISE_FREQUENCY = 0.25;
const float NOISE_AMPLITUDE = 10.0;
const int NOISE_OCTAVES = 3;

float fbm(vec3 p) {
  vec3 q = p + time * 0.25 * vec3(0.0, -0.0, 1.0);
  float g = noise(q);
...
const float FOG_INTENSITY = 0.15;
...
export const VolumetricLightEffect = ({ target }) => {
  const effectRef = useRef();
  const lightRef = useRef();

  const { lightPos, enableBlueNoise, shadowMapSize, enabled } = useControls({
    enableBlueNoise: {
      value: true,
  ...
  const lightPosition = new THREE.Vector3(lightPos.x, lightPos.y, lightPos.z); 
  const lightDirection = useMemo(() => new THREE.Vector3()
      .subVectors(new THREE.Vector3(0, 0, 0), lightPosition)
  ...
  useFrame((state) => {
    ...
    const currentLightTargetPos = new THREE.Vector3().addVectors(lightPosition, lightDirection);
  ...
  return (
    <>
      <mesh ref={lightRef}>
        <spotLight castShadow intensity={80.5} target={target} />
...
const VolumetricLight = () => {
  ...
  const { nodes } = useGLTF('https://cdn.maximeheckel.com/models/arches.gltf');

  const numSteps = 25;
  const maxRadius = 1.85;
  const minRadius = 1.01;
  const discHeight = 0.01;
  const stepVerticalOffset = discHeight;

  const radiusDenominator = numSteps > 1 ? numSteps - 1 : 1;
    ...
    <>
      ...
      <group position={[0, -1.0, 0]}>
        ...
        {Array.from({ length: numSteps }, (_, i) => {
          const yPos = i * stepVerticalOffset;

          const interpolationFactor = i / radiusDenominator;
          const currentRadius =
            maxRadius - (maxRadius - minRadius) * interpolationFactor;

          const safeRadius = Math.max(currentRadius, 0.01);
      ...
      <VolumetricLightEffect target={meshRef.current} />
    </>
  );
};
        ...
        <Suspense>
          ...
          <ambientLight intensity={0.15} />
           <OrbitControls
            ...
            maxAzimuthAngle={Math.PI + Math.PI / 3}
            minDistance={1}
            maxDistance={5}
            enableDamping={true}
            dampingFactor={0.05}
