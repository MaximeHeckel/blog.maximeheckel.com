...
type: 'blogPost'
colorFeatured: 'linear-gradient(315deg, #fde7f9 0%, #aacaef 94%)'
fontFeatured: '#000000'
featured: true
---

import AnimationTypes from '../src/components/FramerGuide/AnimationTypes';
import ClipboardAnimationDetails from '../src/components/FramerGuide/ClipboardAnimationDetails';
import Orchestration from '../src/components/FramerGuide/Orchestration';
...
const CopyToClipboardButton = () => {
  ...
  const clipboardIconVariants = {
    hover: (isClicked: boolean) => ({
      scale: 1.05,
      strokeWidth: 3,
      // This makes sure that the clipboard doesn't show up on hover
...
```
- a `custom` prop passed to the clipboard SVG so we can adapt the variants based on the status of the button
transition objects that we've seen in the first part
- `useMotionValue` and `useTransform` to intertwine the opacity and pathLength values together
```js
const pathLength = useMotionValue(0);
const opacity = useTransform(pathLength, [0, 0.5], [0, 1]);
```
- some **gesture-based animation object** to add a dedicated `hover` and `pressed` animation on the clipboard SVG 
```js
<motion.svg
    initial={false}
    animate={isClicked ? 'clicked' : 'unclicked'}
