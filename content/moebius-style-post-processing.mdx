...
updated: '2024-03-24T08:00:00.000Z'
categories: []
...
**P**hysically **B**ased **R**endering (PBR) is often the _defacto_ stylistic guideline for 3D developers to follow for their creations as
tools/frameworks, for the most part, optimize for realistic materials, shadows, and lighting out-of-the-box.
However, in an industry constantly pushing for realism, adopting stylized shaders while still following physical principles can
...
At least since Zelda BOTW came out, it's becoming more prevalent. While Hi-Fi Rush or the Persona series are great examples of _Non-Photorealistic Rendering_,
...
reproducing some aspect of it in a WebGL environment was possible. Luckily, <Anchor discreet favicon href="https://twitter.com/UselessGameDev">@UselessGameDev</Anchor>
...
that make Jean Giraud's style so unique and rebuilds each of them in Unity. After carefully studying each subject covered in that video and spending a couple of days on the problem,
...
style (outlines, crosshatched shadows, and specular) but also elements usually featured in Moebius' more sci-fi-focused comic books: _desert_, _spaceships_, and _alien planets_
...
In this article, we will deconstruct this art style and reproduce it step-by-step as a post-processing pass that you can put on top of any React Three Fiber scene. We will learn
how to output hand-drawn outlines by leveraging an edge detection filter and stylize our final render while preserving physical elements like lighting and shadows with an extra touch
...
We want to apply the Moebius style to a React Three Fiber scene _as a whole_, not just as a material of the individual meshes that compose it. Knowing that, we'll need to use a **post-processing effect** that can apply this style to an already rendered scene.
A post-processing effect acts like an "image filter" on top of your scene to apply any modifications you'd like. `@react-three/postprocessing` has many effects like Bloom, Vignette, or Depth of Field built in,
...
the scene and a material that takes the underlying scene as input, modifies it, and then renders it on our screen. In this case, you should see that the scene now has a red tint, as we increased the intensity of the red
...
An edge detection filter enables us to **identify points in our scene where the brightness changes sharply**, for instance, between the pixels of an object in the foreground and the background, which should result in us being able
to place our outline around each object pretty accurately. In our case:

- We'll implement a **Solbel Filter**, a type of edge detection filter.
...
_Why would we need the "depth data" of our scene?_ This data will tell us on which plane the objects of our scene belong and thus find any "depth discontinuities" and help us find the _outer_ outlines of the overall scene.

First things first, let's work our way towards getting the depth information of our scene. We can use our good ol' `useFBO` hook to create a render target, but this time, we will also pass a `depth texture` onto it and flip the `depth buffer` option to `true.
...
data that you can read with `texture2D` in GLSL. Luckily, <Anchor discreet favicon href="https://twitter.com/mattdesl">@mattdesl</Anchor> provided a great example on how to read depth texture in [one example](https://threejs.org/examples/?q=depth#webgl_depth_texture)
that he built for the Three.js documentation. Here's the function called `readDepth` that he provides:

```glsl title=readDepth glsl function made my @mattdesl
#include <packing>
...
As we read the pixels of our scene in our post-processing pass, we'll use this filter (which is nothing more than an image filter) to detect sharp changes in _intensity_ (intensity here can be a difference in depth or normals). The Sobel filter is a specific filter that has a few specificities that will help us:
...
Here, we apply the Sobel filter to the depth data we acquired in the previous section. If we were to visualize the output of this post-processing pass, this is what we get:
...
```glsl {15-23,27} title=Applying the Sobel filter to both our Depth data and Normal data
//...
...
Sometimes, the Normal gradient is not enough to draw an outline on the inside of the scene (try to move the camera to point down in the Normal demo and notice that the top of the square blends with the ground). That surfaces in the final scene has "missing outlines." To work around that, we can give "more weight" to the depth gradient.
...
- A `random` function to apply some irregularities to the outline.
...
The demo below implements the displacement we just went through and allows you to tweak our outline through:
...
explores a similar way to achieve a hand-drawn look for Three.js scene via post-processing effect. In it, she proposes an alternative way to handle the displacement of the outline by getting her random value from a texture instead of a hash function. You'll also find a clever technique to do pencil-like shadows 👀.

I recommend reading it if you want to explore more ways to stylize your post-processing shader passes. The Three.js and shader-related content she wrote are among my favorite write-ups on the internet.

</Callout>

### Adding custom shadow patterns to our post-processing effect

To achieve a convincing Moebius style, we want to apply a custom shadow pattern to the shadows and the darker parts of the scene overall. That means that those patterns need to be _coded_ within our post-processing fragment shader, where we'll need to:
...
To solve this, we can reuse the depth buffer we introduced in the first part! We know at which _depth_ a given pixel is through this texture and thus selectively apply the shadow pattern based on that value:
...
- When the specular is over a certain threshold, the output of the fragment shader is `vec4(1.0).` Otherwise, we still return the standard Normal color data.
- I also sneaked in an extra bit of data in place of the alpha: **the diffuse light**.

For that last point, it's more a subjective workaround to the issue I highlighted about using `luma` to detect shaded areas of our scene. However, it falls quickly apart for darker colors. Thus, I included the amount of **diffuse** light when checking the luminance of a pixel with an arbitrary factor (you know me at this point if you've read my shader content, there's always a hacky number somewhere) to ensure that areas with a darker color but lit by our light do not show _too much_ shadow patterns.
...
The white specular reflections that result from this updated code stand out enough from the standard Normal colors enough to get detected by our Sobel filter, giving us lovely outlines around these reflections! The demo below showcases the result of this final touch to our Moebius post-processing pass, which now features:
...
<Image
  ...
  alt="Render of the spaceship mesh used in the final scene (originally made by Sousinho) with a custom version of our Moebius post-processing pipeline returning a blueprint style output"
  width={700}
  height={382}
/>

As a conclusion to this post, I wanted to cover some aspects of [my Moebius-inspired React Three Fiber scene](https://r3f.maximeheckel.com/moebius) I showcased on Twitter in early March 2024. It was interesting to apply the post-processing effect we went through in this article and also uncover _some_ weaknesses of the implementation that I think are worth mentioning.
...
If you've read [Shining a light on Caustics with Shaders and React Three Fiber](/posts/caustics-in-webgl#dynamic-caustics), you may notice something familiar in that code: I'm using the same Normal recomputation logic. Doing this allows the shadows and lights to account for the Perlin noise displacement of the terrain. That is why you see the shadow cast by the spaceship onto the ground follow the shape of the dunes as the scene moves.
...
Thus, the Sobel filter had no way to draw all the outlines of the displaced ground mesh 🫠 which, to its own point of few, was nothing more than a flat plane. That is a clear pitfall of our implementation, and moreover one I only noticed it while building the final showcase of this project...

{<Anchor discreet favicon href="https://twitter.com/Cody_J_Bennett">@Cody_J_Bennett</Anchor>} gave me a couple of alternatives like deriving the Normals from the depth data, or using a `gbuffer` but those seemed a bit too daunting for me at this stage (and to be honest quite tired as well) so I ended up choosing an "the easy way out": **manually traversing the scene and assigning the ground its own custom Normal material**
...
I'm not the most proud of that one 😅, but it works!
...
The demo below features all the techniques and workarounds I mentioned in this last part and showcases a reconstitution of my take on what a Moebius-worthy scene for the web would be.

<MoebiusSandpack scene="scene10" />

Through this attempt at reproducing the Moebius style, we learned how to build **very pleasing stylized shaders** and how we can apply them via post-processing, and on top of that, **with custom shadows, outlines, and lighting patterns**! I hope this exercise gave you confidence and inspiration to go beyond the _physically based_ and emulate different art styles through your shader skills 😄.

Hope you'll have a lot of fun playing with this effect! In the meantime, I'll start exploring other kinds of stylized shaders that are on my bucket list.
