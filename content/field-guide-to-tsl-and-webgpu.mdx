...
- Non–“node materials” appear to be in use, but they’re actually the ones being rendered on screen.
- Scenes containing `ambientLight` or `directionalLight` require adding these lights manually using the `scene`’s `add` method instead of the JSX syntax.
...
I could have written a standalone article for each of the examples and themes presented in this post. However, it felt more natural to showcase them together so that it could act as a guide for anyone looking to transition from classic Three.js/React Three Fiber to this new way to build 3D for the web. I hope this answers some of the questions you may have had regarding WebGPU and TSL, and brings clarity to some aspects that may not be obvious from the available documentation and examples.

If I had to write down a couple of my own thoughts at this point in my learning journey, they would have been the following:

1. The node system is life-changing. I _really_ like it. I think many other developers will agree with me on this one.
2. TSL abstracts _a lot_ of things away, which is both good, as it is very friendly to beginners and lowers the barrier of entry for shader development, but can also be a hassle sometimes, especially the contextual nature of some functions like `instanceIndex`.
3. The abstraction of TSL, while being great for working fast and focusing on building, may prompt me to start working with WebGPU outside of the framework of Three.js/React Three Fiber. There are projects like [TypeGPU](https://docs.swmansion.com/TypeGPU/) that make working with WebGPU more accessible while completely hiding it away from developers.

In the meantime, as some of the details get ironed out, I will still be writing shaders in GLSL unless I have a specific use case where I'd need compute shaders, like in my recent [procedural landscape experiences](https://x.com/MaximeHeckel/status/1954197728524988842), which I'm very excited to tell you more about the day this project is ready.
