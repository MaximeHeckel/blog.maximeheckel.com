...
updated: '2023-03-14T08:00:00.000Z'
categories: []
keywords:
  [
    three.js,
...
web, I get constantly exposed to new tools and techniques, from the diverse React Three Fiber scenes and gorgeous shader materials.
...
In this article, I want to share some of my favorite use cases for render targets. From **creating infinity mirrors**, **portals** to other scenes, **optical illusions**, and **post-processing or transition effects**, you'll learn how to leverage this tool to push your React Three projects above and beyond. 🪄
...
That is the simplest definition I could come up with 😅. In a nutshell, it lets you take a snapshot of a scene by **rendering it** and gives you access to its pixel as a **texture** for later use. Here are some of the use cases where you may encounter render targets:
...
- **Store data**: we can store anything in a render target, even if not necessarily meant to be "rendered" visually on the screen. For example, in particle-based scenes, we can store the poisition of each particles that require large amounts of computation, as it's been showcased in [The magical world of Particles with React Three Fiber and Shaders](/posts/the-magical-world-of-particles-with-react-three-fiber-and-shaders/#going-beyond-with-frame-buffer-objects).

Whether it's for a Three.js or React Three Fiber, creating a render target is straightforward: you need to use the `WebGLRenderTarget` class.
...
For the purpose of this article, we'll keep things simple and only use the `width` and `height` options in some of the code playgrounds.
...
The source code of the `useFBO` hook is accessible in the [@react-three/drei repository](https://github.com/pmndrs/drei/blob/master/src/core/useFBO.tsx).

</Callout>

Now, when it comes to _using_ render targets in a React Three Fiber scene, it's luckily pretty straightforward. Once instantiated, every render target-related manipulation will be done within the `useFrame` hook i.e. **the render loop**.
...
The code playground below showcases what the scene rendered by this code snippet looks like:
...
After seeing this first example you might ask yourself _Why is our scene mapped onto the plane moving along as we rotate the camera?_ or _Why don't we see an empty plane inside the mapped texture?_. Here's a diagram that explains it all:

<Image
  src="blog/first_render_target_2.png"
  alt="Diagram showcasing how the texture of a render target is mapped onto a mesh."
  layout="responsive"
  width={700}
  height={290}
/>
...
That means that any updates to the scene will be taken into account and reproduced onto the plane since we run this loop of taking a snapshot and mapping it continuously.

Now, what if we wanted to fix the empty plane issue? There's, luckily, an easy fix to this: **introduce a second render target** that takes a snapshot of the _first one_ and maps the resulting texture of this up-to-date snapshot onto the plane. The result is an infinite mirror of the scene rendering itself over and over again inside itself ✨.
...
With the code snippet above, we render a sphere in a dedicated scene object called `otherSscene` within `createPortal`. This scene is thus rendered **offscreen** and thus will not be visible in the final render of this component. This has many potential applications, from rendering HUDs or rendering different views angles of the same scene.
...
You may be wondering why `createPortal` has anything to do with render targets. We can use them to render any portalled/offscreen scene to get its texture without even having it visible on the screen in the first place! With this combination, we can achieve one of the first mind-bending use cases (or at least it was to me when I first encountered it 😅) of render targets: **render a scene, within a scene**.
...
There's only one change in this example compared to the first one: instead of calling `gl.render(scene, camera)`, we called `gl.render(otherScene, otherCamera)`. Thus the resulting texture of this render target is of the _portalled scene_ and its custom camera!

_What can you do with this?_ you may ask. Well, what about creating a scene featuring a portal to another scene? 👀🪄
...
```glsl {2-3, 6} title=Simple fragment shader to sample a texture using screen coordinates
...
Now you may wonder what are other applications for using screen coordinates with render targets, so here are two of my favorite scenes that showcase how great they work together!
...
The code snippet bellow showcases how you can achieve this effect:
...
4. We then show mesh1, hide mesh2, and restore the material of mesh3 and mesh4.
5. We finally pass the texture of the render target to another mesh called `lens.
...
This scene is originally inspired by the _stunning_ website of [Monopo London](https://monopo.london/), which has been sitting in my "inspiration bookmarks" for over a year now. I can't confirm they used this technique, but by adjusting our demo to use `@react-three/drei`'s `MeshTransmissionMaterial` we can get pretty close 👀:
...
Yet another portal! I know, I'm not very original with my examples in this article, but bear with me... this one is pretty magical 🪄. A few months ago <Anchor href="https://twitter.com/jesper_vos" favicon discreet>@jesper_vos</Anchor> came up with a scene where a mesh moves through a frame and comes out on the other side with a completely different geometry! It looked as if the mesh \_transformed\* itself while going through this "portal":
...
- some render passes may have unnecessary bloat, thus you _may_ see some performance improvements
...
For the fullscreen triangle, we can use a `BufferGeometry` and manually add the position attributes and uv coordinates.
...
Let's break down the problem. To achieve this kind of transition we need to:
...
- We can pass both render targets' textures to the shader material of a full-screen triangle (identical to the one we used in our post-processing examples) along with a `uProgress` uniform representing the \*progress of the transition: towards `-1` we render `sceneA`, whereas towards `1` we render`sceneB`.
...
Render targets have been challenging to learn for me at first, but since understanding how they work, they have been **the secret tool behind some of my favorite creations**, and I have had a lot of fun working with them so far. I really hope this article will make these concepts _click_ for you to enable you to push your next React Three Fiber/Three.js project further.
