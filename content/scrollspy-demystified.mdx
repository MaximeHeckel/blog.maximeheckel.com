...
My deepest apologies to anyone who tried to read the implementation of my `TableOfContent` component that's currently being used here, it's far from my best code 😅.
...
Scrollspy is a "technique" that's used by developers to **keep track of the content of the user's viewport and highlight the corresponding navigation item**.
...
To answer the second one, we're lucky: the Intersection Observer API allows us to pass a `rootMargin`. **This option adds margins around the root element/the viewport, before computing whether a section is intersecting or not**.
...
- Each section will be _highlighted_ whenever it _intersects_ with the viewport.
...
It can be abstracted, implemented in different ways that are more or less opinionated but for this article, **I'm keeping this close to our main topic on purpose**.
...
1. Create the Intersection and pass a callback function to it. That callback takes 2 arguments `entries` and `observer` but we only need to use the first one in our use-case. `Entries` is an array of objects where **each object describes the intersection of one of the elements that we're observing**.
2. Start observing 🔍! For that, we need to create an array of elements to observe and call `observer.observe(targetElements)`.

<Callout variant="info">

Once you're done "observing" one of the target elements or the whole set you can either call:

- `observer.unobserve(oneOfOurElements)` to stop observing a specific element
...
The second key step for our Hook implementation is to know when we should start observing. For that, we can use `useEffect` so we can start observing as soon as the component using our Hook mounts as follows:
...
1. accessing `current` directly as we do here to observe and disconnect our Intersection Observer is not safe. The `current` we access on mount is not guaranteed to be the same when unmounting.
2. if we were to change the target elements our effect will run again and we'll start keeping track of the new elements which is great! But... we didn't stop keeping track of the older elements. Thus to avoid this scenario from breaking our app, the best thing to do is to check for any existing Intersection Observers currently instantiated and disconnect them before we start observing the new elements:
...
Great, we're almost there! The last step now is to set what to return! For simplicity here, we're only going to **return the index of the target currently intersecting with the viewport**. This index can then be used to retrieve which title to highlight in a table of content component for example.

For that, we can initiate a new state to keep track of the index of the currently active section, and finally implement the callback of our Intersection Observer:
...
- we need the first one so I can set a custom parent element that is not the main viewport for the playground below 😛 (it might also come in handy for you in the future)
...
```ts {13,27-31} title=Implementation of useScrollspy with options
const useScrollspy = (
  elements: Element[],
  options?: {
    offset?: number;
...
- Instead of creating our Intersection Observer in our ref, we simply set it as `null` first
- Then, after disconnecting any pre-existing observers, we create a new one with the current set of options
...
I then added both plugins in my Markdown processor pipeline with my other plugins and _magic_ 🪄 the output generated was exactly what was needed:
...
By clicking on the checkbox below, you can highlight the `<section/>` tags from this blog post, thus visualizing how I sectionize with this method my own blog posts. Try to scroll up and down the post and see how the table of content updates which section is in view!
...
Now it was time to wire everything up. I simply needed to get the ids of the section to pass them to the Hook. There were multiple ways I could have proceeded:
...
Today is your lucky day! I refactored the whole implementation so it's easier and more accessible: [TableOfContent.tsx](https://github.com/MaximeHeckel/blog.maximeheckel.com/blob/next/gatsby-theme-maximeheckel/components/TableOfContent/TableOfContent.tsx)
...
Same! I refactored it as well and isolated it so it's easier to read: [ProgressBar.tsx](https://github.com/MaximeHeckel/blog.maximeheckel.com/blob/next/gatsby-theme-maximeheckel/components/TableOfContent/ProgressBar.tsx)
...
I hope this blog post brought some light on what a Scrollspy is, Intersection Observers, and also how I implemented my table of content components that so many of you seem to like so much (thank you all for the compliments, I _really_ appreciate them 😄).
