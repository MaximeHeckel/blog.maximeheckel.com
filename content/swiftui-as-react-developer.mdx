...
```js title=Basic state management in React using the useState hook
import React from 'react';
...
```swift title=Basic state management in SwiftUI using the @State property wrapper
