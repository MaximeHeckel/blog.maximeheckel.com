...
I had no clue how to build such dynamic meshes or make my geometries move, and my materials change colors. A few Google searches later: I got introduced to the concept of _shaders_ that make scenes like the one above possible, and I wanted to know everything about them. However, **shaders are incredibly difficult**. Thus, I spent the past few weeks studying them, learned new techniques, created [_dozens_ of scenes](https://r3f.maximeheckel.com) from scratch, and hit as many roadblocks.

In this article, you'll find **everything I learned about shaders** during my experimentations, from how they work and use them in a React Three Fiber codebase to making them dynamic and interactive ✨. I included some of my own scenes/shaders as examples, as well as all the resources I used myself and tips on making your shaders composable and reusable.
...
The GLSL code in the demos will be passed as _strings_ as it was easier to make that work with Sandpack.
...
These materials are pretty handy, but sometimes they are very limiting and put boundaries on our creativity. Defining your own material through shaders gives you _absolute control_ over how your mesh looks within a scene. That is why a lot of creative developers decide to create their shaders from scratch!
...
For this first vertex shader example, I showcase how to edit the position of any vertex programmatically by changing the `y` coordinate of each of my vertices and make it a function of the `x` coordinate. In this case, `y = sin(x * 4.0) * 0.2` means that the "height" of our plane geometry follows a [sine curve](https://en.wikipedia.org/wiki/Sine_and_cosine) along the x-axis.
...
It would take a long time to explain why the `gl_Position` formula looks the way it does, but if you want to read more about it, I found the [WebGL model view projection documentation](https://developer.mozilla.org/en-US/docs/Web/API/WebGL_API/WebGL_model_view_projection) from MDN quite useful.

</Callout>

Once the GPU has run the vertex shader and placed all the vertices on the screen, we have the overall "shape" of our geometry, and it can start processing the second function: the **fragment shader**.
...
Using Fragment Shader feels more or less like painting but with computer code. Many creative coders, such as [the author of the Book Of Shaders](https://patriciogonzalezvivo.github.io/glslGallery/), make a lot of stunning effects only through fragment shaders applied to a plane, like paint on a canvas.
...
3. Finally, the worst of all the reasons: when things don't run, nothing renders. You just get a blank screen 😵.
...
- _Do you want to pass the number of ms since the scene rendered?_ That will be through a uniform as well.
...
Then we have to pass the variable to our `ShaderMaterial` through the `uniform` prop as follows:

```jsx {6,13-20,28} title=Example of passing a uniform to a shader
import { Canvas } from '@react-three/fiber';
...
By accessing the uniforms object through the ref of our mesh within the `useFrame` hook and updating any values, we can obtain dynamic uniforms that change their value _through time/each frame_.
...
<Callout variant="danger" label="Watch out for rerenders!">

If your scene contains some React state that can change and thus trigger a rerender: **do not forget to memoize your uniform object**!

If you do not:

- the resulting rerender will create _a new uniform object_
...
I'm not going to deep dive into those in this blog post as I haven't yet got to use them, but I'm mentioning them so you know that they exist.
...
```glsl {2-3,12} title=Using varying to send the value of an attribute to the fragment shader
// vertex shader
...
- We use a combination of the `useFrame` hook from React Three Fiber and uniforms to pass the number of elapsed ms since we rendered the scene.
...
In this part, we'll look at two advanced examples of React Three Fiber scenes with shaders that combine everything we've seen in the previous parts. But first, before we deep dive into those…
...
- <Anchor href="https://twitter.com/winkerVSbecks" favicon discreet>
    @winkerVSbecks
  </Anchor> already wrote an incredible article about this subject, better than I
...
Pretty right ✨? By combining uniforms, varyings, noise, and some hover effects, we created a pretty advanced shader for this scene that is both _dynamic_ and _interactive_.
...
```jsx title=Sample code for a Lamnina custom layer and layered material
...
Worth noting: you sadly can't just copy and paste your shader code into a `CustomLayer` class you have to tweak the code slightly:
...
- I used **Fractal Brownian Motion** that I learned about in [the dedicated chapter of The Book Of Shaders](https://thebookofshaders.com/13/). This noise type can be changed more granularly and produce results that feel more organic, akin to clouds or mountains.
