...
For these so far I've mainly focused on the Serverless Framework's [Serverless for frontend developer's class](https://serverless.com/learn/courses/serverless-for-frontend-developers/) as well as [The Serverless Handbook](https://serverlesshandbook.dev/) from [@Swizec](https://twitter.com/Swizec).
I only used the Serverless framework when learning, I preferred using [Vercel](https://vercel.com) which was, in my opinion, the easiest to use (I still have to checkout how to host serverless functions in Netlify, I'm pretty convinced the UX/DX is similar than on Vercel).
