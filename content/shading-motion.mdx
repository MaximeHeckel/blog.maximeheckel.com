...
subtitle: A deep dive into detecting, extracting, and stylizing motion with WebGPU and TSL, covering motion masking, blob tracking, optical flow, and velocity-based motion blur.
...
This is done on purpose as while preparing this article I noticed that Safari does not support the latter texture type in its current implementation of WebGPU.
...
A fullscreen render pass could produce a similar result at this stage, but compute shaders give us a more flexible foundation for later stages which will transform this "per pixel texture" onto something that doesn't map to screen pixels such as array buffers.
...
<DecorativeMotion variant="blob-tracking" />
...
In my own [Blob Tracking demo](https://r3f.maximeheckel.com/blob-tracking-2), you may have noticed that I also included *sound*. In that scene, blobs play notes based on their position on the screen.
...
<DecorativeMotion variant="optical-flow" />
...
<DecorativeMotion variant="motion-blur" />
...
Since motion blur is applied in screen space, we need the object’s movement across the image rather than across the 3D scene.
