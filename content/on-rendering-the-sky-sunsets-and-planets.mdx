...
Finally, we'll look into Sebastian Hillaire’s LUT-based approach for a more performant result, and my own attempt at implementing it, which was a kind of *stepping outside of my comfort zone* phase for this project.
...
You may have, at some point or another, tried to slap a blue gradient background behind some of your work to give it a more atmospheric look and feel, but doing so never feels quite right <FootnoteRef id="1" />.
 ...
 at first glance, but it’s far from accurate, and it only gets worse if you try to render it at dawn or dusk. If we want physical accuracy, we can’t think of the sky as a flat background texture. For our shader implementation, we must treat it as **the result of light interacting with air and its constituents**, while taking into account several variables, such the altitude of the observer, the amount of dust, the time of day, etc, all of that **in a volume**.
...
<BeforeAfterImage
  ...
  beforeSrc="blog/ozone_before.png"
  afterSrc="blog/ozone_after.png"
...
<BeforeAfterImage
  ...
  beforeSrc="blog/mie_before.png"
  afterSrc="blog/mie_after.png"
  defaultSliderPosition={50}
  width={663}
  height={455}
/>
...
This last version of atmospheric scattering may look quite similar to the one we worked on in the earlier parts of this post, but the underlying process is different: instead of computing every aspect of scattering directly in the final post-processing pass, **we split the work into smaller LUTs that we then compose in the final effect**. Most importantly, we avoid repeatedly raymarching towards the sun in our main raymarching loop, which replaces a costly nested loop with a simple transmittance LUT lookup, resulting in a significant performance boost for the final scene.
