...
I spent the past few months building my [personal website](https://maximeheckel.com) from the ground up, finally taking the time to incorporate some 3D work to showcase what I can do with shaders as soon as it loads.
...
Taking the time to build, combine, and experiment with custom post-processing effects gave me an additional creative outlet, and among the many types I got to iterate on, I always had a particular affection for the several "retro" effects I came up with. With subtle details such as **dithering**, **color quantization**, or **pixelization/CRT RGB cells**, they bring a pleasant contrast between the modern web landscape and a long-gone era of technology we 90s/early 2000s kids are longing for.
...
Given the time I invested dissecting every aspect of these effects, I wanted to dive deep with you into the concepts powering them and the shader techniques I learned along the way. Through this article, I hope to convince you of the power of post-processing effects and that nothing beats an elegant retro vibe applied to a website 👌✨. In it, we'll also look into examples of dithering and pixel art from very talented folks who use the same processes that I'll be introducing later on, as well as some of my own creations that I built while learning about all this.
...
Dithering originated as an early graphics technique to _trick_ the viewer's brain into seeing more color or smoothness in gradients/shadows that the machines back in the day could output by intentionally introducing noise on top of an image or a render. Some of us may have spent some of our childhood fiddling with the graphics settings of early versions of Windows, setting the "number of colors" necessary to run our favorite games. To work around limited color palettes such as 256 or 16 colors, game designers relied a lot on dithering to do the heavy lifting and guarantee that their creations would still render something coherent on the screen and erase any large-scale pattern that may appear such as color banding.

Today, dithering is more an artistic choice than a workaround. Many artists or game designers use this technique as a creative outlet to give their work a unique _retro vibe_ or work within the realms of self-imposed limits in colors. Some great examples of such use of dithering include:

- Basement Studio's [Basement Chronicle](https://chronicles.basement.studio/) game: a well-executed \*point-and-click game that reminds me a lot of my own early gaming experience.
...
<Callout variant="info" label="Erratum">

In my article on the Moebius stylized shader, I opted for a post-processing **pass** instead of an **effect**, justifying this choice by saying that `@react-three/postprocessing` did not support custom shaders as effects. Unfortunately, this was incorrect, so I made the necessary changes to the article.
...
No need to set the `glFragColor`, that is taken care of by the `outputColor` variable that you simply need to set for your effect to be applied on top of your scene.
...
This technique relies on a threshold map defined via a **Bayer Matrix** that contains values used to determine whether we should adjust the color of a given pixel.
...
I used this technique in my raymarched cloud scenes to _"erase the banding or layering effect due to a less granular [raymarching] loop"_ which funny enough is the same use case we need dithering for in our Retro post-processing effect. Unlike the previous techniques, this one relies on a texture that we'll pass to the shader of our custom post-processing effect via a uniform and then sampling it as follows:
...
Don't forget to set your texture wrapping properties after loading the blue noise texture wraps both horizontally and vertically:
...
There are many other dithering techniques that I wanted to explore. Unfortunately some of the most interesting ones like **error diffusion** or **Floyd–Steinberg dithering** are not _fragment shader friendly_ due to their sequential nature.
...
- We calculated our dithering threshold based on the pixel luminance pixel, thus relying on a grayscale version of our scene.
...
Manually setting the colors of our dithering pattern can quickly get out of hand, especially with large color palettes. Instead, we'll use a formula to find the nearest neighboring color of a given pixel color based on the total number of colors we want to output in our effect.
...
In his [video on quantization and dithering](https://www.youtube.com/watch?v=8wOUe32Pt-E&ab_channel=Acerola), <Anchor favicon discreet href="https://twitter.com/Acerola_t">@Acerola_t</Anchor> introduces a technique to map to do just that by using the value of a grayscale color palette to sample a texture defining a custom color palette.
...
The only thing to pay attention to is that the number of color blocks in your palette texture needs to match the number of color you set in your custom effect.
...
I vividly recommend taking the time to read the full article as it goes in-depth into an original and more artistic dithering process. Below you'll find the demo I re-implemented from the blog post which also features some of the missing functions the author did not include in their post.
...
- We snap the UV coordinate to the grid defined by `uv/pixelUV` using the `floor` function.
- We rescale the snapped UV coordinates to the original UV space by multiplying the result by `pixelUV`.
...
Also, we can see that our dithering pattern becomes more visible at a pixel size of 4 and 8, but then, as the scene gets scaled down in resolution, there are not enough pixels in the final output to get any pattern at all.
We'll have to strike the right balance between dithering pattern and pixel size to get the most compelling effect.

With this added to our effect, we have all the ingredients to produce gorgeous pixel art pieces or pixelated 3D scenes! One thing I immediately tried upon wrapping up this post-processing effect was to try it on top of some of my earlier shader work, like in [my scene titled Dithering Waves](https://r3f.maximeheckel.com/dithered-waves) where I applied a grayscale version of it on top of a simple scene rendering a domain distortion texture using Fractal Brownian Motion, inspired by [Inigo Quilez' article](https://iquilezles.org/articles/warp/) on the matter.
...
First and foremost, let's look at how CRT displays work so we can reproduce the effect as accurately as possible. Those monitors have 3 electron guns for each of the color channels (red, green, and blue) that run across the screen and excite their corresponding phosphors on the screen, which in return emit light to form an image. To prevent those beams from hitting the wrong phosphor dots and causing color issues on the final image, CRTs use a **shadow mask** with a metal plate with tiny holes. They can have many different configurations which yield different _mask types_.
...
- We create a cell "offset" for every two cells: some will have an offset of `vec2(0.0)` and some of `vec2(0.0, 0.5)`, thus creating the _staggered effect_ of our shadow mask.
...
We can also include some additional distortion to the UV coordinates of our effect by adding code to the `mainUv` function of our shader. Adding some slight imperfections like these can make our CRT look even more accurate. Below is a simple example, but you can further experiment at your heart's content!
