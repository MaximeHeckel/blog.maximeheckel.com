...
categories: []
keywords: []
slug: /@MaximeHeckel/getting-started-with-typescript-on-gatsby-8544b47c1d27
---

![](img/1__w6a1MKTNTtsrijt8HbHO__g.png)
...
1.  **Typescript config:** this config will live in a file at the root of our Gatsby project and will be named **tsconfig.json**. Here we will set up our compiler options, and which folder to include or exclude in our project when it comes to compiling Typescript files. You can generate a sample config using the `./node_modules/typescript/bin/tsc --init` command.  
    Here’s the config I ended up using on my end, but feel free to tweak it so it fits the needs of your project:

![tsconfig.json example](img/1____JhxVFv2AWd8OdaTCxsZPQ.png)
tsconfig.json example

2\. **Tslint config:** this config will contain our linting rules for our typescript codebase. At the root of our project, we can create a **tslint.json** file and add the following to it (or you can also initialize a config `./node_modules/.bin/tslint —-init`):

![tslint.json example](img/1__u6Q9WFpAlPo6NCOUDvRbZw.png)
tslint.json example
...
3. **gatsby-plugin-typescript and gatsby-plugin-tslint:** here we simply need to add gatsby-plugin-typescript and gatsby-plugin-tslint to the gatsby-config of our project. Other options can be added as well but they are not needed for the rest of this post.

4\. Add the following scripts yo your **package.json**:
...
![Here we can see VSCode highlighting the tslint issues as well as the gatsby cli showing the warnings in the terminal](img/1__tIDz__0NDFheenbNkbMG35w.png)
Here we can see VSCode highlighting the tslint issues as well as the gatsby cli showing the warnings in the terminal
...
![This screenshot shows VSCode highlighting the missing type declaration of a module](img/1__w8E5swN7JC2E7__ww2xSPhg.png)
This screenshot shows VSCode highlighting the missing type declaration of a module
...
![Example of a custom type definition for the gatsby-plugin-google-analytics](img/1__l8BS5C30zyYbva__1eX0VWw.png)
Example of a custom type definition for the gatsby-plugin-google-analytics
...
_A simple global state management package based on React constructs_medium.com](https://medium.com/maxime-heckel/rebuilding-redux-with-hooks-and-context-e16b59faf51c "https://medium.com/maxime-heckel/rebuilding-redux-with-hooks-and-context-e16b59faf51c")[](https://medium.com/maxime-heckel/rebuilding-redux-with-hooks-and-context-e16b59faf51c)

[**Using Flow generics to type generic React components**  
_How Flow generics help typing complex multi-purpose components_medium.com](https://medium.com/maxime-heckel/using-flow-generics-to-type-generic-react-components-22dd642634d8 "https://medium.com/maxime-heckel/using-flow-generics-to-type-generic-react-components-22dd642634d8")[](https://medium.com/maxime-heckel/using-flow-generics-to-type-generic-react-components-22dd642634d8)
