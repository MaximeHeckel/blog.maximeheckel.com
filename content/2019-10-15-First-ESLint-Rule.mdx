...
description: A guide to get started with AST (Abstract Syntax Tree) and custom built ESLint plugins
date: '2019-10-15T14:01:01.595Z'
categories: []
keywords: ['eslint', 'rule', 'testing', 'AST', 'javascript', 'ui', 'linting', 'syntax']
slug: how-to-build-first-eslint-rule
cover: './img/tim-gouw-_U-x3_FYxfI-unsplash.jpg'
type: 'blogPost'
---

When I work with React or more generally with Javascript I always use ESLint for linting. Although I've been very familiar on how to use and configure this tool, I've never actually written a custom ESLint rule from scracth until very recently which sounded first like a daunting task but ended up teaching me a few things. This is what this article is about: how I built this specific rule and how I learned about "Abstract Syntax Tree". Let's dive in it together!
...
To create our custom ESLint rule, we'll need to build a ESLint plugin. Creating a ESLint plugin is similar to creating any other NPM project, with only one specificity: the name of the package needs to start with `eslint-plugin-`.
...
> an AST is simplified and condensed treerepresentation of the structure of source code written in a given programming language. It is "abstract" as it does not represent every detail appearing in the real synta but just the content or structural details.

To build the ESLint rule, we need to get the represention of  the expression `CHECK && yup.validateSync();` in a AST and let the `create` function to return an error everytime the tree for that given expression does not match the valid tree. To find the AST representation of our expression you can use [AST Explorer](https://astexplorer.net), which was very helpful for me.
...
![Here we're highlighting the yup.validateSync() expression to see its AST equivalent](img/ast-explorer.png)

We'll first need ESLint to find that specific node with the object name `yup` and a property name `validateSync` in a `CallExpression`, if found can check one of the parents of that node to see if `CHECK &&` is present. Hence, we can start by writing the following code:
...
The next part of the AST tree that we're looking for is a `LogicalExpression`. We can see from the screenshot above that it's present 2 levels up the tree. We can deduct from this that, if this parent were not to be a `LogicalExpression` our ESLint rule should report an error. We can then continue writing our code snipper above by adding the following:
...
With this code our ESLint rule report an error with the following:
...
This whole process might seem like a lot at first, especially since it involves dealing with AST which isn't the most accessible thing to learn. But, now that we know what is the anatomy of an ESLint rule, we can appreciate even more the insane amount of work done by the community to provide us with all this linting rules that we're using on a day to day basis to make our codebase cleaner and more consistent.
